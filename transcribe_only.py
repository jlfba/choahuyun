...
# -*- coding: utf-8 -*-
"""
FunASR Paraformer 转写脚本
用法:
  python transcribe_only.py              # 自动找最新的Excel
  python transcribe_only.py 2026-07-15   # 指定日期
  python transcribe_only.py all          # 转写所有日期
"""
import re, sys, os
...
def transcribe_one(excel_path, output_path, date_override=None):
    ...
    if "转写模型" in df.columns and df["转写模型"].notna().any():
        ...
        if "Paraformer" in existing:
            log("  跳过 - 已有 Paraformer 转写结果")
            return
...
if __name__ == "__main__":
    ...
    if arg == "all" or arg == "":
        ...
        ep = Path(files[-1])
        op = ep.parent / f"{ep.stem}_FunASR_Paraformer.xlsx"
